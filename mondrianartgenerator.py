...
while True:
    ...
    number_of_segments_to_delete = int(number_of_rectangles_to_paint * 1.5)
    ...
    for i in range(number_of_rectangles_to_paint):
        while True:
            ...
            if canvas[(start_x, start_y)] == WHITE:
                continue
            if (canvas[(start_x - 1, start_y)] == WHITE
               and canvas[(start_x + 1, start_y)] == WHITE):
                orientation = 'vertical'
            elif (canvas[(start_x, start_y - 1)] == WHITE
                  and canvas[(start_x, start_y + 1)] == WHITE):
                orientation = 'horizontal'
            else:
                continue

            points_to_delete = [(start_x, start_y)]
            can_dalate_segment = True
            if orientation == 'vertical':
                for change_y in (-1, 1):
                    y = start_y
                    while 0 < y < height - 1:
                        y += change_y
                        if (canvas[(start_x - 1, y)] == BLACK
                           and canvas[(start_x + 1, y)] == BLACK):
                            break
                        elif ((canvas[(start_x - 1, y)] == WHITE
                              and canvas[(start_x + 1, y)] == BLACK)
                              or (canvas[(start_x - 1, y)] == BLACK
                              and canvas[(start_x + 1, y)] == WHITE)):
                            can_dalate_segment = False
                            break
                        else:
                ...
                for change_x in (-1, 1):
                    ...
                    while 0 < x < width - 1:
                        x += change_x
                        if (canvas[(x, start_y - 1)] == BLACK
                           and canvas[(x, start_y + 1)] == BLACK):
                            break
                        elif ((canvas[(x, start_y - 1)] == WHITE
                              and canvas[(x, start_y + 1)] == BLACK)
                              or (canvas[(x, start_y - 1)] == BLACK
                              and canvas[(x, start_y + 1)] == WHITE)):
                            can_dalate_segment = False
                            break
                        else:
                            points_to_delete.append((x, start_y))
            if not can_dalate_segment:
                continue
            break
    ...
    try:
        input('Press Enter for another work of art, or Ctrl-C to qiut.')
